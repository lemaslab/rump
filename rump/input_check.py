...
class data_check(object):
    ...
    def check_input_balance(self):
        try:
            ...
            exit_with_error('positive file groups are not the same as negative file groups, please check design files.', EXIT_FILE_BALANCE_ERROR)

    def check_input_formats(self):
        try:
            assert(self.get_inputfile_format() == ['mzXML'])
        except AssertionError as exception:
            exit_with_error('not all input files are in .mzXML format, please check input data folders.', EXIT_FILE_TYPE_ERROR)

        
def main():
# Orchestrate the execution of the program
    args = parse_arguments()
    check = data_check(args.pos_data, args.neg_data, args.pos_design, args.neg_design)
